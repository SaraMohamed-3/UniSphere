...
router.get("/dashboard", verifyJWT, professorOnly, async (req, res) => {
  try {
    ...
    const professsorId = professor.professor_id;

    // Get active classes count
    const classesRes = await db.query(
      `SELECT COUNT(*) as count FROM classes WHERE professor_id = $1`,
      [professsorId],
    );
    const activeCoursesCount = classesRes.rows[0].count;
    ...
    const todayClassesRes = await db.query(
      ...
      `,
      [professsorId, dayName],
    );

    const department = professor.department_name || "Faculty";
    ...
    res.json({
      ...
      stats: [
        { label: "Active Classes", value: activeCoursesCount },
...
// GET /api/professor/classes - Get all classes for professor
router.get("/classes", verifyJWT, professorOnly, async (req, res) => {
  try {
    const userId = req.user.user_id;
    ...
    const result = await db.query(
      ...
      LEFT JOIN enrollments e ON c.class_id = e.class_id
      WHERE p.user_id = $1
      GROUP BY c.class_id, co.course_id, co.code, co.name, co.credits
      ORDER BY c.semester DESC, c.day, c.time_start
      `,
      [userId],
    );
...
// GET /api/professor/classes/:classId - Get class details
...
// GET /api/professor/classes/:classId/students - Get students in a class
router.get(
...
// GET /api/professor/classes/:classId/grades - Get all grades for a class
router.get(
...
// POST /api/professor/grades - Add or update a grade
router.post("/grades", verifyJWT, professorOnly, async (req, res) => {
  try {
    ...
    // Verify enrollment belongs to professor's class
    ...
    const existingRes = await db.query(
      ...
      WHERE enrollment_id = $1 AND assessment_type = $2
      `,
    ...
    if (existingRes.rows.length > 0) {
      ...
      result = await db.query(
        `
        UPDATE grades
        SET score = $1, max_score = $2, graded_at = NOW()
        WHERE enrollment_id = $3 AND assessment_type = $4
        RETURNING *
        `,
...
// GET /api/professor/classes/:classId/attendance - Get attendance records
router.get(
...
// POST /api/professor/attendance - Record or update attendance
router.post("/attendance", verifyJWT, professorOnly, async (req, res) => {
  try {
    ...
    const existingRes = await db.query(
      ...
      WHERE enrollment_id = $1 AND class_date = $2
      `,
    ...
    if (existingRes.rows.length > 0) {
      ...
      result = await db.query(
        ...
        WHERE enrollment_id = $3 AND class_date = $4
        RETURNING *
        `,
...
// GET /api/professor/classes/:classId/announcements - Get course announcements
router.get(
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // Verify professor created this announcement
      const annRes = await db.query(
        `
        SELECT ca.announcement_id
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await db.query(
        `DELETE FROM course_announcements WHERE announcement_id = $1`,
        [announcementId],
      );
